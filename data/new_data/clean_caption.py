...
if __name__ == '__main__':
    ...
    with open('./subset_captions_full.json','r') as f:
        cap = json.load(f)
    
    for image_id, caption in cap.items():
        caption = caption.replace(' .', '.').replace(',', '')\
            .replace('1', '<num>')\
            .replace('2', '<num>')\
            .replace('3', '<num>')\
            ...
            .replace('7', '<num>')\
            .replace('8', '<num>').replace('0', '<num>')\
            .replace('<num><num>', '<num>').replace('<num><num>', '<num>').replace('..','.').lower()
        cap[image_id] = caption
    
    with open('clean_subset_captions_full.json','w') as f:
        json.dump(cap,f)
